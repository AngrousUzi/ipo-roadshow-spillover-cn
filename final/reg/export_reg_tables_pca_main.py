"""
Combined PCA output table — selected sheets only.

Sheets written:
  E1_End_PCA    — every-session after-end CAR, stacked top-rivals
  E2_Start_PCA  — every-session after-start CAR, stacked top-rivals
  E3_Before_PCA — every-session before-start CAR (placebo)
...
Output: final/reg/reg_tables_pca_main.xlsx
"""
...
OUT_FILE = Path("final/reg/reg_tables_pca_main.xlsx")

MAX_N = 5

# ─── File stem fragments ──────────────────────────────────────────────────────
_E = "every_after_end_rc1_rc2_ic_pc_yfe_ife_pltfe_w99"
_S = "every_after_start_rc1_rc2_ic_pc_yfe_ife_pltfe_w99"
_B = "every_before_start_rc1_rc2_ic_pc_yfe_ife_pltfe_w99"
...
X_LABEL_MAP = {
    **{f"pc{i}":            f"PC{i}"           for i in range(1, 33)},
    **{f"verbal_pc{i}":     f"Verbal PC{i}"    for i in range(1, 20)},
    **{f"vocal_pc{i}":      f"Vocal PC{i}"     for i in range(1, 20)},
    **{f"visual_pc{i}":     f"Visual PC{i}"    for i in range(1, 20)},
    **{f"visual_fer_pc{i}": f"VisualFER PC{i}" for i in range(1, 20)},
}
...
NOTE_EVERY = (
    ...
    "Columns: AM/PM | Y outcome | n=1..5 PCs.  "
    "R²(ctrl): IPO controls + Year FE + Industry FE.  "
    "* p<0.10  ** p<0.05  *** p<0.01   t-stats in parentheses."
)
NOTE_MEAN = (
    "Columns: Level 1 = group (AM/PM), Level 2 = Y outcome, Level 3 = model with n PCs.  "
    "Each PC row shows its coefficient from the joint n-PC regression; blank = not in that model.  "
    "R²(ctrl): IPO controls + Year FE + Industry FE.  "
    "* p<0.10  ** p<0.05  *** p<0.01   t-stats in parentheses."
)
NOTE_MOD = (
    "Each block = different top-rival subset.  "
    "Rows: PC_i = PC coefficient in moderated+controlled spec; "
    "PC_i×MOD = interaction coefficient; MOD = moderator main effect.  "
    "R²(ctrl): IPO controls + Year FE + Industry FE.  "
    "* p<0.10  ** p<0.05  *** p<0.01   t-stats in parentheses."
)

# ─── Data helpers ─────────────────────────────────────────────────────────────
def melt_pca(df, max_n=MAX_N):
    rows = []
        ...
        for _, row in sub.iterrows():
            for i in range(1, n + 1):
                rows.append({
                    ...
                    "y_col":       row["y_col"],
                    "x_col":       f"pc{i}",
                    "coef_ctrl":   row.get(f"coef_pc{i}_ctrl",   np.nan),
                    "tstat_ctrl":  row.get(f"tstat_pc{i}_ctrl",  np.nan),
                    "pvalue_ctrl": row.get(f"pvalue_pc{i}_ctrl", np.nan),
...
def _build_col_layout(df_long_list, y_cols, groups, max_n):
    ...
    union_lk = {n: {grp: build_lookup(all_df, n, grp) for grp in groups}
                for n in range(1, max_n + 1)}
    x_list = [f"pc{i}" for i in range(1, max_n + 1)]

    def present_ys(grp):
        return [y for y in y_cols
                if any((x, y) in union_lk[n][grp]
                       for n in range(1, max_n + 1) for x in x_list)]
    ...
    for grp in groups:
        ...
        for y in p_ys:
            for ni, n in enumerate(range(1, max_n + 1)):
                ...
                c.fill = PatternFill("solid", fgColor=N_LIGHT[ni])
...
# ─── Mean-regression sheet writer (triple header, n-PC models) ───────────────
def melt_mean_pca(df, max_n=MAX_N):
    rows = []
    for n in range(1, max_n + 1):
        sub = df[df["n_pcs"] == n]
        for _, row in sub.iterrows():
...
def write_sheet_mean(ws, df_long, y_cols, max_n, sheet_title):
    ...
    def present_ys(grp):
        return [y for y in y_cols
                if any((x, y) in lookups[n][grp] for n in range(1, max_n + 1) for x in x_list)]
        ...
        for grp in groups:
            for y in grp_ys[grp]:
                for ni, n in enumerate(range(1, max_n + 1)):
                    ...
                    tc.alignment = C
                    tc.font = Font(size=8, color="595959")
                    tc.fill = PatternFill("solid", fgColor=N_LIGHT[ni])
        row_idx += 2
...
# ─── Mod data helpers ─────────────────────────────────────────────────────────
def melt_pca_mod(df, mod_name, max_n=MAX_N):
    rows = []
    for n in range(1, max_n + 1):
        sub = df[df["n_pcs"] == n]
        for _, row in sub.iterrows():
            for i in range(1, n + 1):
                ...
                rows.append({
                    ...
                    "y_col":           row["y_col"],
                    "x_col":           pc,
                    "coef_pc_mod":     row.get(f"coef_{pc}_mod_{mod_name}_ctrl",   np.nan),
                    "tstat_pc_mod":    row.get(f"tstat_{pc}_mod_{mod_name}_ctrl",  np.nan),
                    "pvalue_pc_mod":   row.get(f"pvalue_{pc}_mod_{mod_name}_ctrl", np.nan),
                    "coef_interact":   row.get(f"coef_interact_{mod_name}_{pc}_ctrl",   np.nan),
                    "tstat_interact":  row.get(f"tstat_interact_{mod_name}_{pc}_ctrl",  np.nan),
                    "pvalue_interact": row.get(f"pvalue_interact_{mod_name}_{pc}_ctrl", np.nan),
            ...
            for grp in groups:
                for y in grp_ys[grp]:
                    for ni, n in enumerate(range(1, max_n + 1)):
                        ...
                        cell = ws.cell(row_idx, col_idx,
                                       fmt_coef(r.get("coef_pc_mod", np.nan),
                                                r.get("pvalue_pc_mod", np.nan))
                                       if r is not None else "")
                        cell.alignment = C; cell.font = Font(size=9)

                        tc = ws.cell(row_idx + 1, col_idx,
                                     fmt_tstat(r.get("tstat_pc_mod", np.nan))
                                     if r is not None else "")
                        tc.alignment = C
                        tc.font = Font(size=8, color="595959")
                        tc.fill = PatternFill("solid", fgColor=light_hex)
...
def melt_pca_from_mod(df, mod_name, max_n=MAX_N):
    """Extract base PC coefs (coef_pc_mod columns) from a moderated regression CSV."""
    rows = []
    for n in range(1, max_n + 1):
        sub = df[df["n_pcs"] == n]
        for _, row in sub.iterrows():
            for i in range(1, n + 1):
                ...
                rows.append({
                    ...
                    "y_col":       row["y_col"],
                    "x_col":       pc,
                    "coef_ctrl":   row.get(f"coef_{pc}_mod_{mod_name}_ctrl",   np.nan),
                    "tstat_ctrl":  row.get(f"tstat_{pc}_mod_{mod_name}_ctrl",  np.nan),
                    "pvalue_ctrl": row.get(f"pvalue_{pc}_mod_{mod_name}_ctrl", np.nan),
...
def _load_pca(base_frag, top_suffix="", mod_key="a_pros_qa"):
    """Load base PC coefs from the mod regression CSV (since base-only reg is not run)."""
    part = f"_{top_suffix}" if top_suffix else ""
    fname = REG_DIR / f"reg_bivariate_grouped_{base_frag}{part}_pca_{mod_key}.csv"
    try:
        return melt_pca_from_mod(pd.read_csv(fname), mod_key)
    except FileNotFoundError:
        print(f"  WARNING: {fname.name} not found — empty block")
        return pd.DataFrame()


def _load_pca_mod(base_frag, mod_key, top_suffix=""):
    part = f"_{top_suffix}" if top_suffix else ""
    fname = REG_DIR / f"reg_bivariate_grouped_{base_frag}{part}_pca_{mod_key}.csv"
    try:
        return melt_pca_mod(pd.read_csv(fname), mod_key)
    except FileNotFoundError:
        print(f"  WARNING: {fname.name} not found — empty block")
        return pd.DataFrame()
...
def _load_mean(fname):
    try:
        return melt_mean_pca(pd.read_csv(REG_DIR / fname))
    except FileNotFoundError:
...
# ─── Load data ────────────────────────────────────────────────────────────────
print("Loading every-session PCA data...")
dfs_end    = [(_load_pca(_E, sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]
dfs_start  = [(_load_pca(_S, sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]
dfs_before = [(_load_pca(_B, sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]

print("Loading a_pros_qa mod data...")
dfs_mod_end    = [(_load_pca_mod(_E, "a_pros_qa", sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]
dfs_mod_start  = [(_load_pca_mod(_S, "a_pros_qa", sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]
dfs_mod_before = [(_load_pca_mod(_B, "a_pros_qa", sfx), lbl, vi) for vi, (sfx, lbl) in enumerate(TOP_VARIANTS)]

print("Loading mean-regression PCA data...")
df_first = _load_mean("reg_bivariate_grouped_mean_firstday_ic_yfe_ife_pca_pltfe.csv")
df_qa    = _load_mean("reg_bivariate_grouped_mean_qa_ctrl_fe_ife_pca_pltfe.csv")
...
for sheet_name, variant_list, y_order in [
    ("E1_End_PCA",    dfs_end,    Y_ORDER_END),
    ("E2_Start_PCA",  dfs_start,  Y_ORDER_START),
    ("E3_Before_PCA", dfs_before, Y_ORDER_BEFORE),
]:
