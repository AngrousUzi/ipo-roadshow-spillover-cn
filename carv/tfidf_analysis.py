"""
Repeat stat + plot analysis using tfidf-filtered competitive firm matching.

For each of 5 similarity dimensions (sim_scope, sim_main, sim_combined,
sim_mda, sim_f1), keep only the top-3.8508% most similar ipo-rival pairs
(threshold = quantile at 1 - 0.038508), then re-run the full pipeline.

Output folders (relative to this script):
  output_tfidf/sim_scope/
  output_tfidf/sim_main/
  output_tfidf/sim_combined/
  output_tfidf/sim_mda/
  output_tfidf/sim_f1/
"""
...
BASE_OUT    = os.path.join(SCRIPT_DIR, "output_tfidf")

DIMS = ["sim_scope", "sim_main", "sim_combined", "sim_mda", "sim_f1"]
...
tfidf = pd.read_csv(
    os.path.join(SCRIPT_DIR, "..", "ind", "ind_all_sim_pairs_within_tfidf.csv"),
)
...
def make_window_figs(ipo, fig_dir, dim_label):
    for fn, period_label, car_cols, cav_cols in WIN_DEF:
        ...
        fig.suptitle(
            ...
            f"(tfidf filter: {dim_label}, with 9:25 auction, est1/2/3)",
...
def make_yearly_figs(ipo, fig_dir, dim_label):
    ...
    for est in ["est1", "est2", "est3"]:
        ...
        fig.suptitle(
            ...
            f"(tfidf filter: {dim_label}, with 9:25 auction)",
            fontsize=11, fontweight="bold",
        )
...
for dim in DIMS:
    ...
    # 1. Threshold & filter tfidf pairs
    threshold = tfidf[dim].quantile(1 - TARGET_RATE)
    kept      = tfidf[tfidf[dim] >= threshold][["stkcd_i", "stkcd_j", "year"]].copy()
    kept.columns = ["stkcd_ipo", "stkcd_rival", "year"]
    print(f"  Threshold: {threshold:.6f}  |  kept pairs: {len(kept):,}  rate: {len(kept)/len(tfidf):.6f}")
    ...
    # attach start_time (IPO-level attribute)
    ipo = ipo.merge(
    ...
    ax.set_title(
        ...
        f"(tfidf filter: {dim}, threshold={threshold:.4f})",
        fontsize=11, fontweight="bold",
    )
